...
- (void)commonInit{
    ...
    _unitColor = [UIColor blackColor];;
    _xLineNameColor = [UIColor blackColor];;
    _yLineValueColor = [UIColor blackColor];;
    _axisLineBackgroundColor = [UIColor whiteColor];;
...
- (void)strokePath{
    ...
    for (NSInteger i = 0; i < _yLineSectionCount; i++) {
        if (_isShowSeparate) {
            [self.layer addSublayer:[self yAxisLineSectionShapeLayer:i sectionLength:self.xLineWidth sectionColor:[UIColor lightGrayColor]]];
        }else{
