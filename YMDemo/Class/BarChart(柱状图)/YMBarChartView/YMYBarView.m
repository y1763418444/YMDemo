...
@interface YMYBarView ()

@property (strong, nonatomic) UIView *separate;

@end

@implementation YMYBarView
...
//    CGSize labelSize = [@"x" sizeWithAttributes:attr];
    ...
    for (int i = 0; i < self.numberOfYAxisElements + 1; i++) {
        ...
        label.width = self.width - self.yAxisTextGap;
        [self addSubview:label];
    }
    
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.width - self.yAxisTextGap, yLabelSize.height)];
    label.textAlignment = NSTextAlignmentRight;// UITextAlignmentRight;
    label.font = self.textFont;
    label.textColor = self.textColor;
