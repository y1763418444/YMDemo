...
@interface YMBarChartView () <YMXBarViewDelegate>

@property (strong, nonatomic) UIScrollView *scrollView;
@property (strong, nonatomic) YMXBarView *xBarView;
@property (strong, nonatomic) YMYBarView *yBarView;
@property (assign, nonatomic) CGFloat gap;

@end

@implementation YMBarChartView
...
- (void)draw {
    ...
    self.xBarView.width = self.xAxisTitleArray.count * (self.barGap + self.barWidth);
    
    self.scrollView.contentSize = self.xBarView.frame.size;
    
    // 给y轴视图传递参数
    self.yBarView.yTitle = self.yTitle;
...
- (void)commonInit {
    ...
    // 添加x轴与y轴视图
    YMYBarView *yBarView = [[YMYBarView alloc] init];
    
    [self addSubview:yBarView];
    
    UIScrollView *scrollView = [[UIScrollView alloc] init];
    
    YMXBarView *xBarView = [[YMXBarView alloc] init];
